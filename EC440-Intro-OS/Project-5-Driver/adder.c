 ...
 *
 *  modified from the chardev.c example file; changes written by
 *  Jonathan Chamberlain for EC 440
 */
...
device_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
        ...
        for (bytes_read = 0; bytes_read < len; bytes_read++) {
          ...
          if (next == '-' && !getTerm) {
            ...
            // non integer character found, including '-' while accumulating
            ...
            // accumulating and reached whitespace or \0 terminator
            if (isNeg) {
              Sum -= Term;
            } else {
              Sum += Term;
